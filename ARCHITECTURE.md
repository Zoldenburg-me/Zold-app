...
- **Primary account** — every user gets one smart-contract account (ERC-4337) on Polygon,
  ...
  - **Crypto-native users**: plain wallet address (deposit USDC/EURe directly,
    or bridge supported tokens into the Polygon account).
...
|---|---|---|---|
| USD | USDC (Polygon, native) | ACH/wire via partner; MoneyGram cash | Deepest liquidity; the settlement backbone |
| EUR | EURe (Monerium on Polygon) | SEPA via Monerium IBAN | Launch EUR rail. EURC is optional only where liquidity and token availability justify it, not a launch dependency |
| CHF | none held at launch | SIC/SEPA via partner FX | CHF stablecoins are too thin (~$41M total cap, ~$40k/day volume across VCHF/ZCHF). Treat CHF as a **display/payout currency**: price it, but settle through USDC/EURe and convert at the edge. Revisit holding ZCHF/VCHF inventory when depth improves |
...
- **Polygon** as the primary chain: Monerium supports EURe there, native USDC
  liquidity is deep enough for launch, fees are low, and mature account
  abstraction tooling is available.
...
- **Stellar** as a *payout rail*, not a home chain: MoneyGram Ramps runs on Stellar
  USDC (SEP-10 auth, SEP-24 deposit/withdraw). We bridge Polygon-USDC → Stellar-USDC
  via Circle (CCTP where supported, Circle Mint redemption/mint otherwise) only at
  payout time.
- **Ethereum/Gnosis** touched only where a provider or treasury operation
  requires them. Launch user balances should live on Polygon so Monerium and the
  on-chain account layer share the same home chain.

## 4. On-chain architecture

```
User (passkey) ─▶ Candide Safe Unified Account (per user, on Polygon)
                     │  gas sponsored via Paymaster
                     │  recovery enabled before real funds
                     │  one-time allowance / policy delegate for sends
                     ├─ USD sub-account  ─┐
                     ├─ EUR sub-account  ─┼─▶ per-currency Vaults (ERC-4626)
                     └─ ...              ─┘        │
                                                   ▼
                                     Treasury / FX Vault (protocol-owned)
                                       ├─ holds USDC/EURe inventory
                                       ├─ executes swaps: DEX aggregator
                                       │   (0x/Uniswap/QuickSwap on Polygon) w/ slippage
                                       │   guards; RFQ for size
...
1. **Smart accounts (ERC-4337)** — Candide Safe Unified Account. Passkey signer
   for consumers, recovery before real funds, and sponsored UserOperations so
   users never touch gas directly.
2. **Per-currency vaults (ERC-4626-style)** — one vault per currency token. A user's
   ...
   accounting.
3. **Allowance / policy delegate** — user-approved, narrowly scoped transfer
   permission. Default sends use one-time allowance for the exact token/amount,
   bound to quote id, recipient commitment, expiry, rail, fee/spread ceiling, and
   refund rules. Recurring allowances are only for explicit scheduled remittances.
4. **FX/Treasury vault** — protocol-owned inventory of USDC + EURe. User-facing FX
...
5. **Bridging module** — Polygon↔Stellar for MoneyGram payouts and treasury
   rebalancing to Ethereum/Gnosis only when required. Isolated, rate-limited, with
   per-day caps.

## 5. Remittance flow (canonical example: US → Kenya cash pickup)

1. Sender funds primary account: ACH → partner mints USDC to their Polygon smart account.
...
4. Payout leg, by recipient's rail:
   - **Bank (EUR)**: swap to EURe if needed, burn via Monerium → SEPA Instant to any IBAN.
   ...
   - **Wallet**: direct USDC/EURe transfer, or bridge to another supported chain.
...
- **Orchestrator** — state machines per transfer (fund → convert → bridge → payout),
  idempotent, with compensation steps (refund path) on partner failure. It prepares
  and relays user-approved operations but does not hold the user's Safe owner key.
...
- Smart accounts are user-controlled (passkey + recovery) → strong argument for
  non-custodial positioning of the on-chain layer. The backend should be a relayer
  and orchestrator, not a signer for user funds. Get counsel per market; the FX
  vault is clearly our regulated activity.
...
- **Phase 0 — corridor MVP (one corridor, e.g. EU→cash-pickup):** Monerium IBAN in
  on Polygon, MoneyGram out. No FX vault yet — route swaps straight through a DEX
  aggregator. Proves the full loop with minimal contracts (smart account + one
  vault).
...
2. **Custody stance** — fully non-custodial smart accounts (harder UX, lighter
   ...
   non-custodial with Candide Safe Unified Accounts, passkeys, recovery, and
   allowance/policy delegates.
