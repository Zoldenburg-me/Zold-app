...
const no = (m: string) => console.log(`  \x1b[31mno\x1b[0m   ${m}`);
...
const orders = orderList(await api.orders()) as any[];
...
ok(`account holds ${eur(balance)} EURe at ${token.address}`);
if (balance <= 0) {
  no("Monerium says it issued, but the chain shows nothing — the mint did not land.");
  process.exit(1);
}
...
if (balance < REDEEM_EUR) {
  no(`only ${eur(balance)} on chain; need at least ${eur(REDEEM_EUR)} to test a redeem`);
  todo("deposit more from the portal, or lower EUR_PROOF_REDEEM_EUR");
  process.exit(2);
}
